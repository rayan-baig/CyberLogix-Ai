...
def test_trial_is_priced_but_not_charged(api, tenant_factory, sensor_factory):
    ...
    assert invoice["monthly_total_usd"] == 99.0
...
def test_decommissioning_reports_what_it_removes(
    ...
    assert resp["billing"]["new_monthly_total_usd"] == 99.0
...
def test_roi_counts_only_answered_incidents(
    ...
    assert answered["subscription_cost_usd"] == 99.0
    assert answered["return_multiple"] == pytest.approx(151.5, abs=0.1)
